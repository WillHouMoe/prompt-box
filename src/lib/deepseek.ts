...
export function buildSystemPrompt(
  ...
  if (mode === "diff") {
    lines.push(
      ...
      "",
      "edits 的硬性规则：",
      "1. find 必须是从「当前内容」里**逐字复制**的一段，一个字都不能变。程序靠它来定位，抄错就改不了。",
      "2. **不要顺手「规范化」原文**。这是最容易出错的地方：",
      "   - 标点必须照抄：原文是「，」就不要写成「,」，原文是「（」就不要写成「(」，原文是「；」就不要写成「;」；",
      "   - 空格、空行、缩进照抄；",
      "   - Markdown 记号照抄：原文有 `**粗体**`、`*` 列表、`#` 标题、`|` 表格、`` `代码` ``，就原样复制，不要删掉也不要换成别的符号；",
      "   - 不要改字、不要调换语序、不要补字、不要用省略号。",
      "3. find 尽量短：一到两行、能唯一定位就够。越长越容易抄错。",
      "4. replace 只写这一段的新版本，不要包含整篇正文，也不要重复 find 之外的内容。",
      "5. 删除内容就把 replace 设为空字符串；新增内容就把附近的原文一起放进 find，再在 replace 里写出合并后的结果。",
      "6. 如果要在整篇末尾追加内容，把 find 设为空字符串、replace 设为要追加的正文。",
      "7. 只改用户要求的地方，不要顺手重写其他地方；改动条数尽量少。",
      "8. 拿不准原文长什么样的地方，就不要写进 edits——漏改一处，用户可以自己补；改错一处，正文就毁了。",
      "9. 如果用户只是在提问、不需要改正文，edits 返回空数组 []。",
      "",
