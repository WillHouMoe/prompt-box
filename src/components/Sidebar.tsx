...
export function Sidebar({
  ...
  const content = (
    <div className="flex h-full flex-col gap-1 overflow-y-auto p-3">
      ...
      <span className="flex items-center justify-between px-1 pb-1">
        <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">标签</span>
        {tags.length > 0 && <span className="text-[11px] text-slate-300">{tags.length}</span>}
      </span>
        ...
        visibleTags.map((tag) => {
          ...
          return (
            ...
            >
              ...
              <span className="shrink-0 text-xs tabular-nums text-slate-400">{tag.count}</span>
            </button>
          )
        })
      )}
