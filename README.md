...
- **变量填充**：使用 `{{variable_name}}` 标记需要填写的内容，自动识别；所有变量都是**高度统一的多行输入框**（5 行起步），内容多了会自己继续长高
- **实时 Preview**：填写变量后即时生成最终 Prompt
- **一键复制**：复制的是替换变量后的最终 Prompt，而不是模板
- **搜索**：标题、内容、分类、标签的实时全文搜索
- **分类**：一级分类，内置「学习 / 编程 / 写作 / 工作 / 生活 / 其他」，可新建也可删除（删除分类时，其中的 Prompt 会移到「无分类」，Prompt 本身不会被删）
- **Chat / Agent 类型**：给 Prompt 打上「网页 Chat」或「本地 Agent」标签，一键区分与筛选
- **Tags**：搜索框下方的「筛选」浮层里有独立的标签区（带 Prompt 数量）可直接筛选；卡片上的标签也能点，再点一次取消筛选
...
- **轻量筛选入口**：没有常驻侧边栏，导航 / 类型 / 分类 / 标签都收在搜索框下方右侧的「筛选」按钮里；当前筛选显示为可一键清除的胶囊
- **瀑布流布局**：桌面最多三列卡片像小红书一样自然交错，不强行拉伸对齐（`src/lib/masonry.ts` 按卡片高度估算分列）
...
```text
src/
  components/      # UI 组件（卡片、弹窗、筛选浮层、命令面板等）
